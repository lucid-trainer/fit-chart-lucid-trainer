...
for (const [i, value] of pos_file_data.entries()) {
  let gyroX = value.gyrox.split(",").pop();
  let gyroY = value.gyroy.split(",").pop();
  let gyroZ = value.gyroz.split(",").pop();
  let row = '[new Date("' + value.timestamp + '"), ' + gyroX + ', ' + 
    gyroY + ', ' + gyroZ + '],';
...
const fileData = readFileSync(require.resolve("./template/gyro-pos.tmp"), { encoding: "utf8" });
